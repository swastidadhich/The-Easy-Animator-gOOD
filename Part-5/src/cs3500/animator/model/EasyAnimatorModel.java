...
public class EasyAnimatorModel implements AnimatorModel {
  ...
  @Override
  public List<Integer> getDiscreteTicks() {
    List<Integer> result = new ArrayList<>();

    for (List<Motion2D> lom : directions.values()) {
      for (Motion2D m : lom) {
        if (! result.contains(m.getTick1())) {
          result.add(m.getTick1());
        }
        if (! result.contains(m.getTick2())) {
          result.add(m.getTick2());
        }
      }
    }
