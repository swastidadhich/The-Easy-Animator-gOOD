...
public class AnimationReader {
  ...
  /**
   * Parses a slo-mo file for the time intervals and tempos.
   * @param readable slo-mo readable file
   * @return a list of speed intervals
   */
  public static List<SpeedInterval> parseSlomo(Readable readable) {
    List<SpeedInterval> result = new ArrayList<>();
    Objects.requireNonNull(readable, "Must have non-null readable source");
    Scanner s = new Scanner(readable);
    // Split at whitespace, and ignore # comment lines
    s.useDelimiter(Pattern.compile("(\\p{Space}+|#.*)+"));
    while (s.hasNext()) {
      String word = s.next();
      if ("slomo".equals(word)) {
        result.add(readSlomo(s));
      } else {
        throw new IllegalStateException("invalid slomo");
      }
    }
    return result;
  }
  ...
  /**
   * Returns a speed interval given a single time interval and tempo in a scanner.
   * @param s scanner
   * @return speed interval
   */
  private static SpeedInterval readSlomo(Scanner s) {

    int[] vals = new int[3];
    String[] fieldNames = {"t1", "t2", "tempo"};
    for (int i = 0; i < 3; i++) {
      vals[i] = getInt(s, "Canvas", fieldNames[i]);
    }

    return new SpeedInterval(vals[0], vals[1], vals[2]);
  }
