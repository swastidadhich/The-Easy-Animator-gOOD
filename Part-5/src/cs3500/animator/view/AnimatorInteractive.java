...
public interface AnimatorInteractive extends AnimatorView {
   ...
   *                                  negative
   */
  void renderOutline(Shape2D shape, Motion2D motion, int tick) throws IllegalArgumentException;
  ...
  /**
   * Set the text for the discrete button showing the current status of discrete play.
   * @param discreteStatus the discrete status, enabled or disabled
   */
  void setDiscreteStatus(boolean discreteStatus);

  /**
   * Set the text for the outline button showing the current status of shape fill/outline.
   * @param outlineStatus the fill or outline status
   */
  void setOutlineStatus(boolean outlineStatus);
  ...
  /**
   * Get this view's tempo (ticks/sec) given a tick.
   * @param tick the tick
   * @return the tempo
   */
  double getTempoAtTick(int tick);
