...
public class InteractiveAnimatorController implements InteractiveController, ActionListener,
  ...
  public void playAnimation() throws IllegalArgumentException {
    ...
    timer = new Timer((int) (1000 / view.getTempoAtTick(tick)), new ActionListener() {
      int time = 1;
      @Override
      public void actionPerformed(ActionEvent e) {
        timer.setDelay((int) (1000 / view.getTempoAtTick(tick)));
        ...
        if (discretePlay) {
          if (! paused && getNextDiscreteTick() == -1 && loop
                  && model.getDiscreteTicks().size() > 0) {
            tick = model.getDiscreteTicks().get(0);
          }
          else if (! paused && getNextDiscreteTick() != -1) {
            tick = getNextDiscreteTick();
          }
        }
        else {
  ...
  public void executeAction(String action) {
    ...
    switch (action) {
      ...
      case "Loop":
        ...
        break;
      case "Discrete":
        discretePlay = ! discretePlay;
        view.setDiscreteStatus(discretePlay);
        break;
      case "Outline":
        System.out.println("a");
        outline = ! outline;
        view.setOutlineStatus(outline);
        break;
      default:
  ...
  private int getNextDiscreteTick() {
    for (Integer i : model.getDiscreteTicks()) {
      if (tick < i) {
        return i;
      }
    }
